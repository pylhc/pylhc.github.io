...
The [Input](input.md) tab (i.e. the landing page) of the GUI provides an interface to input data for the RDTfeeddown analysis.
...
To set up default input and output directories (i.e. default locations that are opened when trying to find input files e.g. models, and where the output files will be saved), create a configuration file named `rdtfeeddown_default.json` (in the current working directory where you will launch the GUI from) with the following structure:

```json
{
  "default_input_path": "/some/path",
  "default_output_path": "/some/other/path"
}
```
...
- ``Xing Knob Name``: The name of the crossing angle knob to use for the analysis in the exact form required by the `Timber` database (not MAD-X naming convention).
- ``RDT``: Accepts just one RDT value.
...
<span class="rdtfeeddown_guilabel" style="--rdtfeeddown_guilabel-bg: green;">Run Analysis</span>: Runs the analysis on the provided data (i.e. collating crossing angle knob values and the RDT values), and displays a list of the output files in the GUI on the [Validation](validation.md) tab.
