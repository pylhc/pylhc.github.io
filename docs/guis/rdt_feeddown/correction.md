...
- [Graph](#__tabbed_1_1) tab: Displays the RDT feeddown data and the correction prediction graphically.

- [Response](#__tabbed_1_2) tab (_optional_ during online analysis if carried out offline beforehand): Allows you to quantify the RDT shift predicted from a specific corrector.
...
=== "Graph Tab"
    ...
    **Main Features:**
    ...
    - <span class="rdtfeeddown_guilabel">Load Files</span>: Load the corrector response files generated by the [Response](#__tabbed_1_2) tab.

    - ``Analysis File to Match``: Select the output file from the [Input](input.md) tab that you want to match with the corrector response.
    ...
    <figure>
    <center>
    <img src="../../assets/images/rdtfeeddown_gui/guimatchf0030.png" width="85%" alt="RDTfeeddown Correction Graph Tab" />
    <figcaption> RDTfeeddown Correction Graph Tab with example of matching </figcaption>
    </center>
    </figure>
...
=== "Response Tab"
    ...
    Similar to the [Input](input.md) tab, this tab allows you to load measurement folders and reference data in the format outputted by the [OMC analysis tools][omc_homepage] to quantify the RDT response for a constant corrector strength between 2 different crossing angles.

    !!! warning "Response Behaviour Assumption"
        This assumes a linear response! This means that the feeddown effect corrected is limited to only be for the linear RDT feeddown e.g. for dodecapolar errors only the decapole is corrected not octupole etc.
